...
class Consumer extends events_1.EventEmitter {
    ...
    pollSqs() {
        ...
        this.receiveMessage(receiveParams)
            ...
            .then(() => {
            if (!this.stopped && (0, utils_1.isPollingReadyForNextReceive)(this.batchSize, this.pendingMessages.length)) {
    ...
    processNextPendingMessage() {
        const message = (0, utils_1.getNextPendingMessage)(this.pendingMessages);
        if (!message) {
            return;
        }
        ...
        this.processMessage(message).then(() => {
            ...
            if (this.pollingStopped && (0, utils_1.isPollingReadyForNextReceive)(this.batchSize, this.pendingMessages.length)) {
    ...
    receiveMessage(params) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield this.sqs.send(new client_sqs_1.ReceiveMessageCommand(params));
            }
            catch (err) {
                throw (0, utils_1.toSQSError)(err, `SQS receive message failed: ${err.message}`);
            }
        });
    }
    deleteMessage(message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                this.pendingMessages = this.pendingMessages.filter((m) => m.sqsMessage.MessageId !== message.MessageId);
                this.emitPendingStatus();
                throw (0, utils_1.toSQSError)(err, `SQS delete message failed: ${err.message}`);
            }
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (this.handleMessageTimeout) {
                    timeout = (0, utils_1.createTimeout)(this.handleMessageTimeout);
                ...
                if ((0, utils_1.isFifo)(this.queueUrl)) {
                    const messages = (0, utils_1.getMessagesByGroupId)(this.pendingMessages, message);
                    // processing has failed, remove all following messages with the same groupId
                    this.pendingMessages = (0, utils_1.filterOutByGroupId)(this.pendingMessages, message);
                    yield this.changeVisibilityTimeoutOfBatch(messages, this.visibilityTimeout, 0);
                }
                else {
                    this.pendingMessages = this.pendingMessages.filter((m) => m.sqsMessage.MessageId !== message.MessageId);
                    yield this.changeVisibilityTimeout(message, this.visibilityTimeout);
                }
                this.emitPendingStatus();
                throw err;
            }
    ...
    changeVisibilityTimeout(message, timeout) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield this.sqs.send(new client_sqs_1.ChangeMessageVisibilityCommand({
                    QueueUrl: this.queueUrl,
                    ReceiptHandle: message.ReceiptHandle,
                ...
                this.emit("error", (0, utils_1.toSQSError)(err, `Error changing visibility timeout: ${err.message}`), message);
            }
        });
    }
    emitPendingStatus() {
    ...
    changeVisibilityTimeoutOfBatch(batch, timeout, elapsedSeconds) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    changeVisibilityTimeoutBatch(messages, timeout) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                return yield this.sqs.send(new client_sqs_1.ChangeMessageVisibilityBatchCommand(params));
            }
            catch (err) {
                this.emit("error", (0, utils_1.toSQSError)(err, `Error changing visibility timeout batch: ${err.message}`), messages);
            }
        });
    }
    startHeartbeat() {
        this.heartbeatTimeout = setInterval(() => __awaiter(this, void 0, void 0, function* () {
            const now = Date.now();
            const batches = (0, utils_1.groupMessageBatchByArrivedTime)(this.pendingMessages);
            for (const batch of batches) {
                const elapsedSeconds = Math.ceil((now - batch[0].arrivedAt) / 1000);
                const timeout = this.visibilityTimeout || 0;
                yield this.changeVisibilityTimeoutOfBatch(batch, timeout, elapsedSeconds);
            }
