...
class Consumer extends events_1.EventEmitter {
    ...
    processBatchPendingMessages() {
        const messages = utils_1.getNextPendingMessageBatch(this.pendingMessages);
        if (!messages.length) {
            return;
        }
        const now = Date.now();
    ...
    processMessageBatch(messages) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                yield this.executeBatchHandler(sqsMessages);
                const processedTime = Date.now();
                this.emit("message_batch_processed", sqsMessages, {
                    processingStartedAt: processingStart,
                    processedAt: processedTime,
                });
                yield this.deleteMessages(sqsMessages);
            }
            catch (err) {
                this.emitError(err, sqsMessages);
                if (this.terminateVisibilityTimeout) {
                    yield this.changeVisibilityTimeoutBatch(sqsMessages, 0);
                }
            }
        });
    }
    ...
    deleteMessages(messages) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield this.sqs.send(new client_sqs_1.DeleteMessageBatchCommand(deleteParams));
            }
            catch (err) {
                throw utils_1.toSQSError(err, `SQS delete message failed: ${err.message}`);
            }
                ...
                // delete from pending messages
                this.pendingMessages = utils_1.removeMessagesFromPending(this.pendingMessages, messages);
                this.emitPendingStatus();
            }
        });
    }
    executeHandler(message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (utils_1.isFifo(this.queueUrl)) {
                    const messages = utils_1.getMessagesByGroupId(this.pendingMessages, message);
    ...
    executeBatchHandler(messages) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (this.handleMessageTimeout) {
                    timeout = utils_1.createTimeout(this.handleMessageTimeout);
                    yield Promise.race([this.handleMessageBatch(messages), timeout.pending]);
                }
                else {
                ...
                const pendingMessages = utils_1.getMessagesByGroupId(this.pendingMessages, message);
                // processing has failed, remove all following messages with the same groupId
                this.pendingMessages = utils_1.filterOutByGroupId(this.pendingMessages, message);
                yield this.changeVisibilityTimeoutOfBatch(pendingMessages, this.visibilityTimeout, 0);
                this.emitPendingStatus();
                throw err;
            }
            finally {
    ...
    emitError(err, messageOrMessages) {
