...
const getNextPendingMessage = (batch) => {
...
exports.getNextPendingMessage = getNextPendingMessage;
const filterOutByGroupId = (pendingMessages, msg) => {
...
exports.filterOutByGroupId = filterOutByGroupId;
const getMessagesByGroupId = (pendingMessages, msg) => {
    return pendingMessages.filter((m) => {
        var _a, _b, _c;
...
const groupMessageBatchByArrivedTime = (batch) => {
    return [...new Set(batch.map((w) => w.arrivedAt))].map((arrived) => batch.filter((w) => w.arrivedAt === arrived));
};
exports.groupMessageBatchByArrivedTime = groupMessageBatchByArrivedTime;
const isPollingReadyForNextReceive = (batchSize, pendingSize) => {
    return pendingSize + Math.min(10, batchSize) <= batchSize;
};
exports.isPollingReadyForNextReceive = isPollingReadyForNextReceive;
